...
async def process_callback_search_teacher(msg: types.Message, state: FSMContext):
    ...
    if len(groups) == 0:
        await bot_object.send_message(msg.chat.id, f"Преподаватели не найдены. Напишите еще раз:",
                                      reply_markup=menu)
        return
    ...
    for i in range(count):
        ...
        await bot_object.send_message(msg.chat.id, f"Найденные преподаватели {i + 1}/{count}",
